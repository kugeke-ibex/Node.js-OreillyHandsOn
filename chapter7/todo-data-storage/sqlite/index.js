...
const { join } = require('paht')
const sqlite3 = process.env.NODE_ENV === 'producation'
    ? require('sqlite3')
...
exports.fetchByCompleted = completed =>
    dbAll('SELECT * FROM todo WHERE completed = ?', completed).then(rows => rows.map(rowToTodo))
...
exports.update = (id, update) => {
    ...
    return dbRun(`UPDATE todo SET ${setColumns.join()} WHERE id = ?`, values)
        .then(({ changes }) => changes === 1
            ? dbGet('SELECT * FROM todo WHERE id = ?', id).then(todos => todos.map(rowToTodo))
            : null
        )
}
